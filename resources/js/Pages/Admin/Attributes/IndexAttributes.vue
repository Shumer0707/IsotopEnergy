...
watch(() => usePage().props.flash?.success, (newVal) => {
    if (newVal) {
    ...
    if (confirm('Удалить атрибут?')) {
        router.delete(`/admin/attributes/${id}`, {
            onSuccess: () => {
                ...
                router.visit('/admin/attributes', { only: ['attributes'], preserveScroll: true });
            }
        });
    }
};
                ...
                <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                    ...
                    <div v-if="viewMode === 'list'">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-lg font-semibold">Список атрибутов</h3>
                            <button
                        ...
                        <table class="w-full border-collapse border border-gray-300">
                            <thead>
                                <tr class="bg-gray-200">
                                    ...
                                    <th class="border border-gray-300 px-4 py-2">RU</th>
                                    <th class="border border-gray-300 px-4 py-2">RO</th>
                                    <th class="border border-gray-300 px-4 py-2">EN</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="attr in attributes" :key="attr.id">
                                    <td class="border border-gray-300 px-4 py-2">{{ attr.id }}</td>
                                    <td class="border border-gray-300 px-4 py-2">{{ attr.name_ru }}</td>
                                    <td class="border border-gray-300 px-4 py-2">{{ attr.name_ro }}</td>
                                    <td class="border border-gray-300 px-4 py-2">{{ attr.name_en }}</td>
